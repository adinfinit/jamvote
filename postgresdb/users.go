...
func (repo *Users) Create(ctx context.Context, cred *auth.Credentials, u *user.User) (user.UserID, error) {

	var userid int64
	err := repo.db.QueryRowContext(ctx, `
		INSERT INTO credentials provider, email, name)
		VALUES (@provider, @email, @name)
		RETURNING userid
	`, sql.Named("provider", cred.Provider),
		sql.Named("email", cred.Email),
		sql.Named("name", cred.Name)).Scan(&userid)
	if err != nil {
		return 0, err
	}
